...
The github.io PWA tracker is retired (2026-07-16): `index.html` is a static signpost and `tracker-inline.js` is the single tracker copy. Session hand-off between machines is done with the tracker's backup export/import instead of the old `#dipi=` hash URL.
...
| File | Role |
|---|---|
| `scraper.js` | Scrapes the DataTable on dipi search-course pages (statuses incl. the WaitList/Review pool; reloads first if the page's `?s=` filter is narrower). Offers "Open Inline Call Tracker". Exposes `window.DipiScraper.run()` for re-scrape from tracker. |
...
| `scraper.user.js` | Tampermonkey shell. Adds the Scrape FAB (click-to-run; right-click opts into auto-run) and answers the letter-fetch bridge via `GM_xmlhttpRequest`. `@updateURL` set, so logic edits propagate without reinstall. |
| `scraper-bookmarklet.txt` | Scraper-only bookmarklet. Sets `_DIPI_TRACKER_BASE` (and a now-inert `_DIPI_PWA_URL`). |
...
| `index.html` | Static landing page at github.io — signposts to the on-page tools. The old PWA tracker it hosted is retired. |
| `background.js` | Extension (MV3) service worker: letter-fetch bridge (l.php sends no CORS headers). |
...
To copy a session to another browser/machine, use Export → "💾 Backup session (JSON)" and "📂 Import backup…" (merges by AID, newest progress wins).
...
- **Chrome Web Store packaging — remote-code policy (learned from rejection "Blue Argon",
  ...
  (3) plain page links (wa.me / `whatsapp://` deep links) and data fetches
  (l.php letters via the background worker) are fine — the ban is on remote *code*, not remote data or navigation.
  Store zip = extension files only, built with:
  `zip -r dipi-tools-<version>.zip manifest.json background.js extension-fab.js scraper.js tracker-inline.js course-audit/loader.js course-audit/audit.js photo-review/review.js photo-review/facematch.js vendor/mediapipe vendor/faceapi icons`
...
The PWA at github.io worked but required navigation away from dipi. On mobile this means lost context, fresh page load, and re-typing the URL to come back. Inline rendering keeps everything in one tab. The PWA was kept as a fallback for a while, then retired on 2026-07-16 once the inline tracker covered everything (backup export/import replaced the cross-machine hash hand-off).
...
### Why preserve PWA fallback? (historical — retired 2026-07-16)

(1) Existing bookmarklets in the field shouldn't break. (2) Some users may not have Tampermonkey. (3) The PWA could be opened on a different machine via the `#dipi=...` hash URL. All three are now covered by the bookmarklet/extension paths and the session backup export/import, so the duplicate tracker copy was removed.
...
- ~~Cross-origin session sync~~ — superseded by session backup export/import (shipped 2026-07-16)
- Diff mode in re-scrape: highlight what changed since last run (TODO.md #2)
- Stats panel: avg attempts to confirm, time-of-day patterns, etc.
- ~~Unit tests on tracker logic~~ — `test/tracker.test.js` covers dates/merge/backfill (the DOM import path itself is still untested)
